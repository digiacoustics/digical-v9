// Set initial timestamp in footer
...
function calcRoomAreas(r) {
  ...
  // Walls: wall1 and wall3 = length*height, wall2 and wall4 = width*height
  const wall1 = l * h,
    wall2 = w * h,
    wall3 = l * h,
    wall4 = w * h;
  ...
  for (const d of r.deductions) {
    let dl = parseFloat(d.len) || 0,
      dh = parseFloat(d.ht) || 0;
    deductionArea += dl * dh;
  }
...
function saveProject() {
  ...
  localStorage.setItem('DIGICAL-' + data.projectName, JSON.stringify(data));
  alert('Project saved!');
}
